...
def simulate(s, dt, params: SystemParams, commands, key):
    ...
    dw = random.normal(key, (3,))
    noise_new = noise_old - params.OU_THETA * noise_old * dt + params.OU_SIGMA * jnp.sqrt(dt) * dw
    ...
    out_of_bounds = jnp.logical_or(jnp.abs(pos_old[0]) > 0.355, jnp.abs(pos_old[1]) > 0.6085)
    ...
    noise_body = quat_new.inv().apply(noise_new)
    F_net_body = gravity + thrust_vector - drag + noise_body
    F_net_world_frame = quat_new.apply(F_net_body)

    normal_z_mag = jnp.where(F_net_world_frame[2] < 0, -F_net_world_frame[2], 0.0)
    ground = pos_old[2] <= -table_dropoff
