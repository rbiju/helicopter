...
int convert_analog_to_command(int cmd, bool is_throttle, bool is_trim) {
  int out;
...
void loop() {
  ...
  inputBuffer[1] = convert_analog_to_command(analogRead(YAW), 0, 0);
  inputBuffer[2] = convert_analog_to_command(analogRead(PITCH), 0, 0);
  inputBuffer[3] = convert_analog_to_command(analogRead(THROTTLE), 1, 0);
  inputBuffer[4] += convert_analog_to_command(analogRead(TRIM), 0, 1);
